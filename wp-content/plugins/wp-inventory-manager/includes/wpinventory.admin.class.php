...
final class WPIMAdmin extends WPIMCore {
	...
	public static function save_labels() {
		...
		$include_in_sort = self::request( "include_in_sort" );
	...
	public static function edit_display() {
        ...
        <form method="post" action="<?php echo self::$self_url; ?>">
            ...
            <p class="wpim_notice"><?php echo sprintf(self::__( 'Drag-and-drop items from the Available Fields list to the desired list to control what shows on the front-end inventory. %sView the explainer video%s.' ), '<a href="https://www.youtube.com/watch?v=3a72VtNFmWw&t=16s" target="_blank">', '</a>' ); ?></p>
            <div class="lists">
	...
	public static function edit_settings() {
        ...
        <form method="post" action="<?php echo self::$self_url; ?>" class="inventory-config">
            ...
            <h3 data-tab="license_keys"><?php self::_e( 'License Keys' ); ?></h3>
            <table class="form-table">
                <tr class="subtab">
                    <th colspan="2"><h4 data-tab="license_keys"><?php self::_e( 'License Keys' ); ?></h4></th>
                </tr>
                <tr>
	...
	public static function update_reg_key( $settings = [] ) {
		...
		// Triggered by the "Validate Licenses" button in the Settings page
		...
		foreach ( $add_ons AS $add_on => $data ) {
			...
			if ( $reg_key ) {
				...
				// Or it's not valid
				if ( empty( $reg_info['valid'] ) || ! $reg_info['valid'] ) {
					$check_key = TRUE;
				}

				if ( $scheduled_update ) {
					$check_activate = $check_key;
					$check_key      = TRUE;
				}
			} else {
			...
			if ( $check_key ) {
				$action = ( $check_activate ) ? 'activate' : 'check';
				WPIMAPI::activate( $data, $reg_key, $action );
			}
		}
	}
