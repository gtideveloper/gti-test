...
class WPIMShortcodes {
	...
	private static function args( $name ) {
		$args                  = self::$args;
...
function wpinventory_category_widget_shortcode( $args, $instance ) {
	...
	foreach ( $categories AS $category ) {
		...
		if ( $list ) {
			$content .= '<li class="category_' . $category->category_id . ' category_' . $wpim_categories->get_class( $category->category_name ) . '">';
			$content .= '<a href="' . $category_link . '">' . $category->category_name . '</a>';
			$content .= '</li>';
		} else {
			$content .= '<option value="' . $category_link . '">' . $category->category_name . '</option>';
		}
	}
