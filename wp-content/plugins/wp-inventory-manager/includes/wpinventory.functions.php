...
function wpinventory_get_reserve_config( $args = [] ) {
	...
	if ( ! $inventory_id ) {
		$inventory_id = ( ! empty( $_POST['_wpim_inventory_id'] ) ) ? $_POST['_wpim_inventory_id'] : NULL;
	}
...
function wpinventory_process_reserve( $data ) {
	...
	$note = apply_filters( 'wpim_reserve_email_message', $note, $data );
	if ( trim( $note ) ) {
		$position = (int) wpinventory_get_config( 'reserve_email_message_position' );
		$position = apply_filters( 'reserve_email_message_position', $position, $data );
		if ( ! $position ) {
			$message = '<p>' . $note . '</p>' . PHP_EOL . $message;
		} else {
			$message = $message . PHP_EOL . '<p>' . $note . '</p>';
		}
...
function wpinventory_filter_form( $args = NULL ) {
	...
	if ( $search ) {
		...
		$form .= ( $search_label ) ? '<label>' . $search_label . '</label>' : '';
		$form .= '<input type="text" name="inventory_search" value="' . stripslashes( $inventory_search ) . '" />';
		$form .= '</span>' . PHP_EOL;
	}
	...
	if ( $sort ) {
		...
		$form .= ( $sort_label ) ? '<label>' . $sort_label . '</label>' : '';
