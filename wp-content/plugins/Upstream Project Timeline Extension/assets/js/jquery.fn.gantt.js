/**
 * This library was slightly changed by Denison Martins <contact@denison.me>.
 *
 *
 * jQuery Gantt Chart
 *
...
(function ($, undefined) {
    ...
    $.fn.gantt = function (options) {
        ...
        var settings = {
            ...
            onResizeOrMove: function (data, direction, startDate, endDate) { return; },
            onRender: $.noop,
            startDate: 0,
            endDate: 0
        };
        ...
        var core = {
            ...
            init: function (element) {
                ...
                // Add the attribute for referencing the bar which is in resizing behavior and the required data.
                element.resizingBar = null;
                element.resizingSide = null;
                element.newStartDate = null;
                element.newEndDate = null;
                element.days = null;
                ...
                if (element.data.length > 0) {
                    ...
                    // Interval in days between start and end dates.
                    var daysInterval = Math.ceil(((+element.dateEnd / 1000) - (+element.dateStart / 1000)) / DAY_IN_SECONDS);

                    var estimateVisibleDays = Math.ceil($(element).width() / 32) - 7;
                    if (daysInterval < estimateVisibleDays) {
                        element.dateEnd.setDate(element.dateEnd.getDate() + (estimateVisibleDays - daysInterval));
                    }

                    /* core.render(element); */
                    core.waitToggle(element, function () { core.render(element); });
                } else {
                    $(element).addClass('u-no-items');

                    element.rowsNum = 1;
                    element.pageCount = 1;
                    element.rowsOnLastPage = 1;
                    element.dateEnd = new Date();

                    setTimeout(function () {
                        // Try to estimate the number of days visible plus a week based on current wrapper's length.
                        var estimateVisibleDays = Math.ceil($(element).width() / 32) + 7;

                        element.dateEnd.setDate(element.dateEnd.getDate() + estimateVisibleDays);
            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage &&
                        ...
                        var html =
                            ...
                            (entry.type === 'subitem' ? ' sml' : (' fn-wide ' + dataId)) +
                            '" id="rowheader' + i +
                        ...
                        if (entry.type === 'subitem') {
                            var assignedTo = (typeof entry.assigned_to !== 'undefined') ? entry.assigned_to : '';

                            entries.push(
                                '<div class="row subitem row' + i +
                                ' " id="RowdId_' + i + dataId + '">' +
                                '<span class="fn-label">' +
                                entry.label +
                                '</span>' +
                                '<div class="fn-assigned">' +
                                assignedTo +
                                '</div>' +
                                '</div>'
                            );
                ...
                entries.push(
                    '<div class="row spacer" style="height:5px;width:100%;border-top:1px solid #ddd"></div>'
                );
            ...
            getCoreElement: function (elem) {
                return $(elem).parents('.fn-gantt').parent()[0];
            },

            getColFromXCoord: function (x, element) {
                var $dataPanel = $(element).find('.fn-gantt .dataPanel');
                var cellSize = tools.getCellSize();
                var corrY;

                // Find the Y value for the respective week number cell in the header, according to the current
                // scale.
                ...
                return tools.elementFromPoint(x, $dataPanel.offset().top + corrY);
            },

            getDateFromXCoord: function (x, element) {
                var col = core.getColFromXCoord(x, element);

                if (col === null) {
                    return false;
                }

                return parseInt(col.parentElement.dataset.repdate);
            },
            ...
            dataPanel: function (element, width) {
                ...
                $dataPanel.click(function (e) {
                    ...
                    var col = core.getColFromXCoord(e.pageX, element);
                    ...
                    var row = tools.elementFromPoint($leftpanel.offset().left + $leftpanel.width() - 10, e.pageY);
                    // Was the label clicked directly?
                ...
                // Handle resize movements and dispatch to registered `onResizeBar` function
                $dataPanel.on('mousedown', '.left-handler', function (e) {
                    if (!$(e.currentTarget).hasClass('left-handler')) {
                        return;
                    }
                    ...
                    var element = core.getCoreElement(e.currentTarget);

                    element.resizingBar = $(e.currentTarget).parents('.bar');
                    element.resizingSide = 'left';
                    element.updating = false;
                    element.newStartDate = parseInt(element.resizingBar.data('startdate'));
                    element.newEndDate = parseInt(element.resizingBar.data('enddate'));
                    element.days = parseInt(element.resizingBar.data('days'));

                    element.resizingBar.addClass('resizing');
                });

                $dataPanel.on('mousedown', '.right-handler', function (e) {
                    if (!$(e.currentTarget).hasClass('right-handler')) {
                        return;
                    }
                ...
                $dataPanel.on('mousemove', function (e) {
                    var element = core.getCoreElement(e.currentTarget);

                    // Ignore if we are not resizing any bar.
                    if (element.resizingBar === null) {
                        return;
                    }
                    ...
                    if (element.resizingSide === 'left') {
                        // Move to the left: start date.
                        left = $(core.getColFromXCoord(e.pageX, element)).position().left;

                        element.resizingBar.css('left', left);

                        element.updating = true;

                        element.newStartDate = core.getDateFromXCoord(e.pageX, element);
                        element.newEndDate = element.newStartDate + (days * oneDayInMs) - oneDayInMs;
                    } else if (element.resizingSide === 'right') {
                        // Move to the right: end date.
                        element.updating = true;

                        element.newEndDate = core.getDateFromXCoord(e.pageX, element);

                        // Calculate the new date interval in days.
                        days = Math.round(Math.abs(element.newEndDate - element.newStartDate) / (oneDayInMs)) + 1;
                        element.days = days;

                        width = days * tools.getCellSize();
                        element.resizingBar.css('width', width);
                    }
                });

                $dataPanel.on('mouseup', function (e) {
                    var element = core.getCoreElement(e.currentTarget);
                    ...
                    element.resizingBar.find('.left-handler, .right-handler').removeClass('hovering');
                    element.resizingBar.removeClass('resizing');
                    ...
                    if (element.updating) {
                        if (!element.newStartDate || !element.newEndDate) {
                            return;
                        }

                        core.waitToggle(element, function () {
                            settings.onResizeOrMove(
                                element.resizingBar,
                                element.resizingSide,
                        ...
                        // Just in case the page do not reload, we update the element data.
                        element.resizingBar.data('startdate', element.newStartDate);
                        element.resizingBar.data('enddate', element.newEndDate);
                        element.resizingBar.data('days', element.days);
                    }
                });
            ...
            createProgressBar: function (days, progress, label, desc, color, dataObj, startDate, endDate) {
                ...
                var $bar = $(
                    '<div style="background: ' + color + '" class="bar ' + dataObj.type + '" data-content="' + desc + '" data-startdate="' + startDate + '" data-enddate="' + endDate + '" data-days="' + days + '"' + '>' +
                    '<div class="left-handler"></div>' +
                    '<div class="right-handler"></div>' +
                    '<div class="fn-label">' + label + '</div>' +
                    '<span style="width:' + progress + '%" class="progress-bar"></span>' +
                    '</div>'
                );
                ...
                if (typeof dataObj === 'object') {
                    var data = Object.entries(dataObj);
                    var item = [];
                    if (data.length > 0) {
                        for (var i = 0; i < data.length; i++) {
                            item = data[i];

                            $bar.data(item[0], item[1]);
                        }
                    }
                }
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                case 'hours':
                                    ...
                                    _bar = core.createProgressBar(dl, day.progress, day.label, day.desc, day.color, day.dataObj, dFrom, dTo);
        ...
        var tools = {
            ...
            getMaxDate: function (element) {
                if (settings.endDate > 0) {
                    return new Date(settings.endDate * 1000);
                }
            ...
            getMinDate: function (element) {
                if (settings.startDate > 0) {
                    return new Date(settings.startDate * 1000);
                }
